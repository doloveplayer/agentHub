...
> **版本**：v0.3.0  
> **状态**：Phase 1/2 完成（权限代理基础设施已交付），Phase 3 规划中  
> **拟定人**：AgentHub 项目团队  
> **日期**：2026-05-18 · 修订 2026-05-19（最终）
...
- [x] **@ 提及智能提示**
  ...
  - [x] 根据上下文推荐 Agent（已交付，2026-05-21）
...
- [x] **权限代理机制**（基础设施已交付，完整交互式 → [#10](https://github.com/doloveplayer/agentHub/issues/10)）
  - `permission_request` 事件已被 EventParser 解析
  - 后端 `write()` 通过 `/proc/pid/fd/0` 向 Claude Code 进程 stdin 投递 y/n 响应
  - 多 Agent 权限路由：permissionId 格式 `{agentMessageId}|::|{tool}|::|{timestamp}`，精确路由到目标 Agent
  - 120s 权限超时自动 deny；session 清理时取消全部待处理超时
  - 前端交互式权限卡片（Allow/Deny 按钮 + resolved 状态追踪）
  - Trust 模式复选框（默认开启，关闭后进入权限代理模式）
  - Agent Card 活动流渲染 permission_request 事件（🔐 图标 + 琥珀色背景）
  - **已知限制**：Claude Code CLI 在 `--print` 模式下不发出 `permission_request` JSON 事件（工具权限静默跳过）。当前 trustMode=false 实际为沙箱安全模式。完整交互式权限需 PTY 方案或等 Claude Code 上游支持（见 [#10](https://github.com/doloveplayer/agentHub/issues/10)）
  - 前端 401 时自动清除 token 并跳转登录页（`api.ts:20-23`）
  - **实现文件**: `apps/api/src/ws/handler.ts`, `apps/api/src/agent/ClaudeCodeProcess.ts`, `apps/api/src/agent/SandboxManager.ts`, `packages/shared/src/types.ts`, `apps/web/src/components/ChatView.tsx`, `apps/web/src/components/MessageInput.tsx`, `apps/web/src/components/AgentCard.tsx`, `apps/web/src/hooks/useChat.ts`, `apps/web/src/store/appStore.ts`
...
每个会话可设置信任级别，通过前端 Trust 复选框控制（默认开启）：

| 模式 | 行为 | 适用场景 |
|------|------|---------|
| 正常 (Trust OFF) | 不带 `--dangerously-skip-permissions`，权限请求经用户确认 | 日常使用 |
| 信任 (Trust ON) | `--dangerously-skip-permissions`，跳过所有权限确认，依赖沙箱隔离兜底 | 熟悉环境、高风险操作已知的场景 |
...
**已交付**：
- EventParser 解析 `permission_request` JSON 事件
- 后端 WebSocket 广播权限请求（permissionId 路由、120s 超时 auto-deny）
- 前端交互式权限卡片（Allow/Deny 按钮 + resolved 状态）
- `/proc/pid/fd/0` 权限响应投递
- 多 Agent 权限路由（permissionId 嵌入 agentMessageId）
- Trust 模式复选框

**已知限制**：Claude Code CLI 在 `--print` 非交互模式下不发出 `permission_request` JSON 事件（工具权限被静默跳过）。完整交互式权限代理跟踪在 [#10](https://github.com/doloveplayer/agentHub/issues/10)。
...
| 2: 多 Agent | 2-3 周 | 群聊 + @ 指令 + / 透传 + Agent 实时活动流 + 终止控制 + 权限代理基础设施 | ✅ 完成（完整交互式权限 → [#10](https://github.com/doloveplayer/agentHub/issues/10)） |
| 3: Orchestrator | 3-4 周 | Planner + DAG 任务卡片 + BullMQ 调度 + 失败处理 | 🟡 进行中（Tier 0-1 完成，Tier 2 待完成） |
| 4: 全流程增强 | 按依赖顺序，不设时限 | Diff → 预览 → 审查 → 部署 → 测试 → 依赖检查 | 🔜 |

### 10.3 阶段 1 详细任务分解

**后端任务**
- [x] 初始化 monorepo 项目结构（`apps/api`、`apps/web`、`packages/shared`）
- [x] 搭建 Hono 服务，基础中间件（CORS、日志、错误处理、JWT 验证）
- [x] 实现 GitHub OAuth 登录流程（/auth/github → /auth/callback → JWT 签发）
- [x] 白名单中间件：检查 GitHub username 是否在允许列表
- [x] 集成 Prisma，定义 User、Session、Message、Agent、SessionAgent 模型
- [x] 实现会话 CRUD API（创建、列表、详情、删除）
- [x] 实现消息持久化（发送时写入 DB，加载历史时读取）
- [x] 搭建 WebSocket 服务（/ws），JWT 认证连接，心跳保活
- [x] 实现 Claude Code 适配器：
  - ClaudeCodeProcess：spawn 子进程，设置 cwd 为沙箱路径
  - EventParser：解析 stream-json 行，分类处理
  - 流式输出通过 WebSocket 推前端
- [x] 使用 Dockerode 为会话创建沙箱容器（挂载独立 bind-mount workspace）
- [x] 会话关闭/超时后自动清理容器和子进程

**前端任务**
- [x] 用 Vite 初始化 React + TypeScript 项目，配置 Tailwind
- [x] 实现 GitHub OAuth 登录页和回调处理
- [x] 简易聊天 UI 组件：ChatView（消息列表 + 输入框）、MessageBubble（user/agent 样式）
- [x] 流式打字机效果：WebSocket 接收 chunk → 逐字追加到 Agent 气泡
- [x] 会话列表组件（SessionList）：展示标题、最新消息预览、新建/切换
- [x] useChat hook：管理 WebSocket 连接、消息发送、流式接收、消息状态
- [x] 前后端联调，确保链路通畅

**DevOps**
- [x] 编写 docker-compose.yml：API 服务、PostgreSQL、Redis
- [x] 配置 Nginx/Caddy 反向代理模板
- [x] 确保 Docker 宿主机可被 API 容器访问（挂载 Docker socket）
