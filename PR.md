...
- [ ] **@ 指令解析引擎**
  - 后端解析消息中的 `@AgentName`，分割为多段指令，每段路由到对应 Agent
  - 解析规则：`@` 开头匹配已注册 Agent 名称，遇下一个 `@` 或字符串结束为边界
  - 未 @ 任何 Agent 的消息默认发给会话绑定 Agent（或上次交互的 Agent）
  - 同一消息中可 @ 多个 Agent，并行启动对应子进程

- [ ] **/ 指令透明透传**
  - 用户输入 `/` 开头的消息（如 `/brainstorming`、`/plan`）**原样转发**给 Claude Code 子进程 stdin
  - AgentHub 不做拦截、解析或修改——由 Claude Code 自行识别和执行 skills/commands
  - 前端可选：输入 `/` 时弹出补全面板，列出已知 Claude Code commands/skills（数据来自后端缓存的 Claude Code 配置），但仅用于提示，不改变消息内容

- [ ] **群聊界面**
  - 消息按发送者分组显示（Human / CodeAgent / ReviewAgent / DevOpsAgent，各带头像 + 名称）
  - 人类用户用 GitHub 头像，Agent 用机器人图标 + 首字母
  - 群聊信息栏展示参与者列表及在线状态（在线/忙碌/离线）
  - 消息气泡根据发送者使用不同颜色或标识

- [ ] **@ 提及智能提示**
  - 输入 `@` 弹出 Agent 选择面板，支持模糊搜索和键盘导航（↑↓ 选择，Enter 确认）
  - 根据上下文推荐 Agent（如输入 "review" 优先出现 ReviewAgent）
  - 面板显示 Agent 的简要能力描述和当前在线状态

- [ ] **多会话并行**
  - 左侧会话列表支持同时打开多个会话标签
  - 可在不同会话间快速切换，每个标签独立维护 WebSocket 连接
  - 未读消息红点提示

- [ ] **权限代理机制**
  - Claude Code JSON 事件流中的 `permission_request` 事件被后端捕获
  - 转换为消息卡片（含操作描述、文件路径、命令内容）推送到前端
  - 用户点击"允许"或"拒绝"→ 后端将响应写回 Claude Code stdin
  - 每个会话可设置信任级别：
    - **正常模式**：所有权限请求经用户确认
    - **信任模式**：(需要安全确认开启，仅供高级用户使用) 跳过权限确认，依赖沙箱兜底

- [ ] **Agent 状态面板（右侧上下文面板）**
  - 每个会话显示活跃 Agent 列表，每个 Agent 展示：
    - 运行状态（🟢运行中 / 🟡等待权限 / ⚪空闲）
    - 思考等级（`max` / `standard`）
    - 当前正在执行的操作（如"正在执行 Bash: npm test"）
    - 上下文用量（tokens 使用量 / 限制，进度条）
    - 已打开/修改的文件列表
    - 活跃子 Agent 列表（Claude Code 内部派生的 sub-agent）
  - 标签页切换：[文件树] [Agent状态] [任务卡片（阶段3）]
  - 状态数据来源：后端 EventParser 解析 Claude Code stream-json 事件，StateTracker 维护内存快照

- [ ] **上下文隔离**
  - 每个会话独立的工作目录和沙箱容器
  - 文件系统变化仅影响当前会话
  - 同一会话内的多个 Agent 共享同一文件系统

- [ ] **WebSocket 增强**
  - 支持按会话分组的多路复用
  - 消息类型区分：chat（聊天）、permission（权限请求）、state（状态更新）、system（系统通知）

**交互原型**：  
> 用户创建群聊"我的全栈项目"，手动添加 Agent 成员。输入 `@CodeAgent 写一个 Express 服务器` → Agent 气泡开始流式输出代码... → 输入 `@ReviewAgent 检查刚才的代码` → ReviewAgent 读取文件系统中的代码，输出审查意见。右侧面板实时显示两个 Agent 的状态。
...
| 阶段 | 周期 | 核心产出 |
|------|------|---------|
| 1: MVP | 2-3 周 | 单 Agent 流式聊天 + Docker 沙箱 + GitHub OAuth |
| 2: 多 Agent | 2-3 周 | 群聊 + @ 指令 + / 透传 + 权限代理 + Agent 状态面板 |
