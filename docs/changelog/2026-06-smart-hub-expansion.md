...
## Turn-Based Message Operations (06-10)

**目标：** 引入 ConversationTurn 模型，解决群聊/多 Agent 协作场景下消息操作语义不明确的问题

**关键产出：**
- `ConversationTurn` 数据模型：一次用户意图触发的一个完整多 Agent 协作周期
- Turn 级删除：级联清理消息 + workspace git 回滚 + ContextEntry 标记 stale + PlanExecution 取消 + QuoteReference/PinnedMessage 清理
- Turn 级重新生成：workspace 回滚到旧 Turn 快照 → 创建子 Turn（parentTurnId 版本链）→ 前端版本切换器
- 单 Agent 撤回：软删除（content 清空 + turnStatus='undone'），UndoPlaceholder 占位符 UI，不影响同 Turn 内其他 Agent 消息
- 前端 Turn 分组渲染：TurnBoundary 分隔线 + VersionSwitcher 版本导航
