...
- 已通过测试模式缓解：0
- 未修复/待确认缺陷：0
- Mock Provider 扩展 (2026-05-25)：新增高频 chunk、迟到 chunk、错误含 secret、stop 生命周期、无沙箱错误、并发队列 6 种 mock 行为；新增 `apps/api/src/apiEdgeCases.test.ts` 含 24 条 API 测试
- 单元/集成测试总数：101（`node:test`），全部通过
