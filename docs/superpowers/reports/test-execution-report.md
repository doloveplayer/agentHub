...
- GitHub OAuth 测试账号标识：白名单用户名 `JohnSiegfried`，非白名单用户名 `hengming0820`；`JohnSiegfried` 已通过 Playwright 可见浏览器完成真实 GitHub OAuth 回调，DB 创建用户记录，`/api/auth/me` 对该用户 JWT 返回 200；`hengming0820` 按用户要求暂不执行。
...
权限代理在 test provider 下已可回归，但真实 Claude Code provider 的 permission_request 能力、凭据文件/日志脱敏、Planner DAG 完整确认执行、外部 OAuth 非白名单拒绝用例、云部署、长时压测和人工视觉类用例仍是主要未覆盖风险。
