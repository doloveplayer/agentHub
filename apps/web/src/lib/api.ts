...
export function appendTokenParam(url: string): string {
...
export const api = {
  login: (username: string, password: string) =>
    request<{ token: string; userId: string; username: string }>(
      "/auth/login",
      {
        method: "POST",
  ...
  sendMessage: (
    sessionId: string,
    content: string,
    mentions?: { agentId: string; agentName: string; subPrompt: string }[],
  ) =>
  ...
  validateSkillFile: (file: File) => {
    ...
    return request<import("@agenthub/shared").SkillValidationResult>(
  ...
  createAgentFromMd: (
    ...
    request<any>("/agents/from-md", {
      method: "POST",
      body: JSON.stringify({ content, providerConfig }),
    }),

  getProviderConfigs: () =>
    request<Record<string, { apiKey?: string; endpoint?: string }>>(
      "/agents/provider-configs",
    ),

  saveProviderConfigs: (
    configs: Record<string, { apiKey?: string; endpoint?: string }>,
  ) =>
    request<{ success: boolean }>("/agents/provider-configs", {
      method: "PUT",
      body: JSON.stringify(configs),
    }),

  getWorkspaceTree: (sessionId: string) =>
    request<{
      tree: any[];
      workspaceTree: any[];
      sandboxDir: string;
  ...
  getWorkspaceFile: (sessionId: string, path: string) =>
    request<any>(
      `/workspace/${sessionId}/file?path=${encodeURIComponent(path)}`,
    ),

  updateWorkspaceFile: (sessionId: string, path: string, content: string) =>
    request<{ path: string; size: number; modifiedAt: string }>(
      `/workspace/${sessionId}/file`,
      {
        method: "PUT",
  ...
  downloadWorkspacePath: async (sessionId: string, path: string) => {
    ...
    const res = await fetch(
      `${BASE_URL}/workspace/${sessionId}/download?path=${encodeURIComponent(path)}`,
      { headers },
    );
    if (!res.ok) {
      if (res.status === 401) {
  ...
  getWorkspaceChanges: (sessionId: string) =>
    request<{ changes: string[] }>(`/workspace/${sessionId}/changes`),

  // Workspace configuration
  getSessionWorkspace: (sessionId: string) =>
    request<{ path: string | null; mode: string; writePermission: string }>(
      `/sessions/${sessionId}/workspace`,
    ),

  setSessionWorkspace: (
    sessionId: string,
    config: { path: string; mode?: string; writePermission?: string },
  ) =>
    request<{
      success: boolean;
      path: string;
  ...
  browseDirectory: (dirPath: string) =>
    request<{ path: string; dirs: { name: string; path: string }[] }>(
      `/workspace/browse?path=${encodeURIComponent(dirPath)}`,
    ),

  getDiffFiles: (sessionId: string, baseVersionId?: string) =>
    request<{ files: any[] }>(
      `/diff/${sessionId}/files${baseVersionId ? `?baseVersionId=${encodeURIComponent(baseVersionId)}` : ""}`,
    ),

  getFileDiff: (sessionId: string, path: string, baseVersionId?: string) =>
    request<{ file: any }>(
      `/diff/${sessionId}/file?path=${encodeURIComponent(path)}${baseVersionId ? `&baseVersionId=${encodeURIComponent(baseVersionId)}` : ""}`,
    ),

  getVersions: (sessionId: string) =>
    request<{ versions: any[] }>(`/diff/${sessionId}/versions`),

  createVersion: (
    sessionId: string,
    body: { agentName?: string; summary?: string },
  ) =>
    request<{ version: any }>(`/diff/${sessionId}/versions`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  diffVersions: (sessionId: string, from: string, to: string, path?: string) =>
    request<{ diff: any }>(
      `/diff/${sessionId}/versions/diff?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}${path ? `&path=${encodeURIComponent(path)}` : ""}`,
    ),

  restoreVersion: (sessionId: string, versionId: string) =>
    request<{ ok: boolean }>(`/diff/${sessionId}/restore`, {
      method: "POST",
      body: JSON.stringify({ versionId }),
    }),

  acceptDiffFile: (sessionId: string, path: string) =>
    request<{ ok: boolean }>(`/diff/${sessionId}/accept`, {
      method: "POST",
      body: JSON.stringify({ path }),
    }),

  rejectDiffFile: (sessionId: string, path: string, baseVersionId?: string) =>
    request<{ ok: boolean }>(`/diff/${sessionId}/reject`, {
      method: "POST",
      body: JSON.stringify({ path, baseVersionId }),
    }),

  getPreviewPorts: (sessionId: string) =>
    request<{ ports: number[] }>(`/preview/${sessionId}/ports`),

  serveStaticDir: (sessionId: string, directory: string) =>
    request<{ port: number; directory: string; proxyUrl: string; url: string }>(
      `/preview/${sessionId}/serve-static`,
      {
        method: "POST",
  ...
  forwardPreviewPort: (sessionId: string, port: number) =>
    request<{
      ...
      url: string;
      proxyUrl: string;
    }>(`/preview/${sessionId}/forward`, {
      method: "POST",
      body: JSON.stringify({ port }),
    }),

  capturePreviewScreenshot: (sessionId: string, url: string) =>
    request<{ image: string; capturedAt: number }>(
      `/preview/${sessionId}/screenshot`,
      {
        method: "POST",
  ...
  generateDeployConfig: (
    ...
    request<{ files: string[] }>(`/deploy/${sessionId}/config`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  deployToPlatform: (
    ...
    request<{ deploymentId: string; status: string }>(
      `/deploy/${sessionId}/run`,
      { method: "POST", body: JSON.stringify(body) },
    ),

  rollbackDeployment: (sessionId: string) =>
    request<{ ok: boolean; output: string }>(`/deploy/${sessionId}/rollback`, {
      method: "POST",
    }),

  runSecurityAudit: (sessionId: string) =>
    request<{ report: any; exitCode: number }>(`/security/${sessionId}/audit`, {
      method: "POST",
    }),

  upgradeDependencies: (sessionId: string, packageName?: string) =>
    request<{ output: string; exitCode: number }>(
      `/security/${sessionId}/upgrade`,
      { method: "POST", body: JSON.stringify({ packageName }) },
    ),

  deleteMessage: (messageId: string) =>
    request<{ ok: boolean }>(`/chat/messages/${messageId}`, {
      method: "DELETE",
    }),
  ...
  removeAgentFromSession: (sessionId: string, agentId: string) =>
    request<void>(`/sessions/${sessionId}/agents/${agentId}`, {
      method: "DELETE",
    }),
  ...
  getQuoteReferences: (messageId: string) =>
    request<{ quotedFrom: any[]; quotedBy: any[] }>(
      `/quote-references?messageId=${encodeURIComponent(messageId)}`,
    ),
