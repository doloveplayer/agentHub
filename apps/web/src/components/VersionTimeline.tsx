  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-xs font-medium uppercase tracking-wide text-hub-tertiary">Version history</div>
        <button onClick={refresh} className="text-xs text-hub-muted hover:text-hub-secondary">Refresh</button>
      </div>
      {versions.length === 0 && (
          ...
          <div key={version.id} className="rounded-md border border-hub bg-hub-hover px-3 py-2">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-xs font-medium text-hub-primary">{version.summary}</div>
      ...
      {versions.length >= 2 && onCompare && (
        <div className="space-y-2 rounded-md border border-hub p-2">
          <div className="grid grid-cols-2 gap-2">
            ...
            <select value={to} onChange={(e) => setTo(e.target.value)} className="rounded bg-hub-code px-2 py-1 text-xs text-hub-secondary">
              {versions.map((version) => <option key={version.id} value={version.id}>{version.summary}</option>)}
            </select>
          </div>
          <button
