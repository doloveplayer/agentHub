...
try {
  ...
  if (result.count > 0) {
    console.log(
      `[startup] Reset ${result.count} stale streaming message(s) to done`,
    );
  }
} catch {
  /* DB might not be ready or prisma not connected yet */
}
...
nodeServer.on("upgrade", (req, socket, head) => {
  // Authenticate WebSocket upgrade via ?token= query param
  try {
