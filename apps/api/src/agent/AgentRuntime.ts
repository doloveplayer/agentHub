...
class AgentRuntime {
  ...
  async ensureRunning(agentId: string, sandbox?: SandboxInfo): Promise<AgentEntry> {
    ...
    if (sandbox) {
      ...
      AgentDirectoryManager.initialize(sandbox.hostSandboxDir, agent.name, agent.systemPrompt, null, undefined, agent.skills as any[] | null);
      ...
      if (!agent.containerId || agent.containerStatus === 'stopped') {
        ...
        AgentDirectoryManager.initialize(info.hostWorkDir, agent.name, agent.systemPrompt, null, undefined, agent.skills as any[] | null);
        await prisma.agent.update({
          where: { id: agentId },
