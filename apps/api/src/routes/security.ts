...
security.post('/:sessionId/audit', async (c) => {
  ...
  let output = '';
  const result = await SandboxManager.execStream(containerId, ['sh', '-lc', 'npm audit --json'], {
    workDir: '/workspace',
    onStdout: (chunk) => { output += chunk; },
    onStderr: (chunk) => { output += chunk; },
  });
  const report = parseNpmAuditJson(extractJson(output));
  broadcast(sessionId, { type: 'security_report', report, exitCode: result.exitCode, timestamp: Date.now() });
...
security.post('/:sessionId/upgrade', async (c) => {
  ...
  broadcast(sessionId, { type: 'security_upgrade', output, exitCode: result.exitCode, timestamp: Date.now() });
