...
export function readWorkspaceFileTreeForTest(
  workspaceRoot: string,
  source: "sandbox" | "workspace" = "sandbox",
): FileNode[] {
...
workspace.get("/:sessionId/file", async (c) => {
  ...
  try {
    ...
    const message =
      status === 400 || status === 403 ? err.message : "Failed to read file";
...
workspace.put("/:sessionId/file", async (c) => {
  ...
  try {
    ...
    const message =
      status === 400 || status === 403 ? err.message : "Failed to write file";
    return c.json({ error: message }, status as any);
  }
});
